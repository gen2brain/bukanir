...
func getConfig() string {
	...
	flag.StringVar(&config.Trackers, "trackers", "udp://tracker.publicbt.com:80,udp://tracker.openbittorrent.com:80,udp://open.demonii.com:80,udp://tracker.istole.it:80,udp://tracker.coppersurfer.tk:80,udp://tracker.leechers-paradise.org:6969,udp://exodus.desync.com:6969,udp://tracker.pomf.se,udp://tracker.blackunicorn.xyz:6969,udp://pow7.com:80/announce", "Additional trackers (comma-separated URLs)")
	flag.IntVar(&config.ListenPort, "listen-port", 6881, "Use specified port for incoming connections")
	flag.IntVar(&config.TorrentConnectBoost, "torrent-connect-boost", 500, "The number of peers to try to connect to immediately when the first tracker response is received for a torrent")
